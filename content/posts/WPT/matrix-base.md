...
Let $\mathbf{A}, \mathbf{B}$ be a square matrices of order n.
...
If A is a (real) n x n symmetric matrix, then A has n real eigenvalues (counted bytheir multiplicities). For each eigenvalue, we can find a real eigenvector associated with it.

### Proof

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/realEigenvalues.png" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Proof of all eigenvalues of a real symmetric matrix are real.
</div>
...
This is obviously true for every 1x1 matrix A if $$ A = [a] $$, then 
$$ E_1^{-1} (a) E_1 = (a) $$

Assume now that every (n - 1) x (n - 1) real symmetric matrix is orthogonally diagonalizable.

Consider an n x n real symmetric matrix A where n > 1. By the preceding theorem, we can find a real eigenvalue λ of A, together with a real eigenvector $$ v_1 $$. By normalizing, we can assume 
